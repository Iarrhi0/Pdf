...
const VERSION = '3.0.0-render-stream';
const PORT = Number(process.env.PORT || 10000);
const HOST = process.env.HOST || '0.0.0.0';
const MAX_CONCURRENCY = clamp(Number(process.env.CSPDF_MAX_CONCURRENCY || 3), 1, 3);
...
async function imageMetaFile(file, bytes, sha256){ try{ const m=await sharp(file,{failOn:'none',sequentialRead:true}).metadata(); const w=m.width||0,h=m.height||0; if(w<250||h<350||w*h<150000)return null; if(bytes<12000&&w*h<500000)return null; return{width:w,height:h,format:(m.format||'').toLowerCase(),sha256}; }catch{return null;} }
let sharpBusy=false; async function toPdfImageFile(input, meta, output){ if(meta.format==='jpeg'||meta.format==='jpg'||meta.format==='png')return input; while(sharpBusy)await sleep(25); sharpBusy=true; try{await sharp(input,{failOn:'none',sequentialRead:true}).rotate().jpeg({quality:90,mozjpeg:false}).toFile(output);return output;}finally{sharpBusy=false;} }
...
class TaskManager{
  ...
  async run(t){let tmpPdf='',stream=null,pdf=null;const workDir=path.join(TMP,`work-${t.id}`);try{await fsp.mkdir(workDir,{recursive:true});t.started=now();t.state='LOAD';const page=await fetchHtml(t.url);t.url=page.finalUrl;if(t.cancelled)throw new Error('__CANCELLED__');t.state='ANALYZE';const ex=extractImages(page.html,page.finalUrl);t.engine=ex.engine;t.total=ex.urls.length;if(t.total<2)throw new Error(`Seulement ${t.total} image(s) candidate(s)`);tmpPdf=path.join(TMP,`${t.id}.pdf.part`);pdf=new PDFDocument({autoFirstPage:false,compress:true,margin:0,bufferPages:false});stream=fs.createWriteStream(tmpPdf,{highWaterMark:64*1024});pdf.pipe(stream);const seen=new Set();let validBytes=0,lastTaskBytes=0,lastTaskAt=now();t.state='DOWNLOAD';for(let i=0;i<ex.urls.length;i++){await this.waitIfPaused(t);const rawFile=path.join(workDir,`${String(i+1).padStart(4,'0')}.img`);const converted=path.join(workDir,`${String(i+1).padStart(4,'0')}.jpg`);try{const got=await fetchImageToFile(ex.urls[i],page.finalUrl,page.cookies,rawFile);const meta=await imageMetaFile(rawFile,got.bytes,got.sha256);if(!meta||seen.has(meta.sha256)){t.ignored++;}else{seen.add(meta.sha256);const pdfImg=await toPdfImageFile(rawFile,meta,converted);const pageW=595,pageH=Math.max(842,pageW*meta.height/Math.max(1,meta.width));pdf.addPage({size:[pageW,pageH],margin:0});pdf.image(pdfImg,0,0,{fit:[pageW,pageH],align:'center',valign:'center'});t.valid++;validBytes+=got.bytes;}t.bytes_done+=got.bytes;this.totalBytes+=got.bytes;}catch(e){if(e.message==='__CANCELLED__')throw e;t.ignored++;}finally{await removeQuiet(rawFile,converted);}t.processed=i+1;t.progress=t.processed/Math.max(1,t.total);const ts=now();if(ts-lastTaskAt>=600){t.speed_bps=(t.bytes_done-lastTaskBytes)/((ts-lastTaskAt)/1000);lastTaskBytes=t.bytes_done;lastTaskAt=ts;}if(rssMb()>=MEMORY_HARD_MB)await sleep(350);}
    t.state='PDF';pdf.end();await new Promise((resolve,reject)=>{stream.on('finish',resolve);stream.on('error',reject)});const ratio=t.valid/Math.max(1,t.total),avg=validBytes/Math.max(1,t.valid);if(t.valid<2||(t.total>=5&&ratio<0.60)||(t.valid>=10&&avg<18000))throw new Error(`Qualité insuffisante: ${t.valid}/${t.total} vraies pages`);const final=path.join(TMP,`${t.id}-${downloadName(t.series,t.number)}`);await fsp.rename(tmpPdf,final);tmpPdf='';t.output=final;t.state='SAVED';t.ended=now();setTimeout(()=>removeQuiet(final),60*60*1000).unref();
  }catch(e){try{if(pdf&&!pdf._ended)pdf.end();}catch{}if(tmpPdf)await removeQuiet(tmpPdf);if(e.message==='__CANCELLED__'){t.state='STOPPED';t.error='Arrêté';}else{t.state='ERROR';t.error=e.message||String(e);}t.ended=now();}finally{try{await fsp.rm(workDir,{recursive:true,force:true})}catch{}}}
